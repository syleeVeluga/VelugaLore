...
- 2026-04-29 보강: `packages/desktop/src/desktop-session.test.ts` 에 §13.7.3 9단계를 단일 세션으로 종주하는 tracer 테스트(`walks the PRD §13.7.3 nine-step /draft tracer in a single session`)를 추가했다. 이 테스트는 `agent-server` subprocess + `/draft` 호출 + approval queue + 2-phase write + 외부 편집 watcher propagation 을 한 번에 검증하므로, 이후 사람 손 smoke 는 동일한 동선이 Tauri 창에서 그대로 재현되는지만 확인하면 된다.

자동화로 닫힌 게이트:

- 승인된 patch 의 2-phase write 결과와 `body_sha256` 일치 — `desktop-session.test.ts` 의 `/draft` 흐름 + 9단계 tracer 두 곳에서 디스크 본문 SHA-256 과 `result.document.bodySha256` 일치를 단언한다.
- 외부 markdown 편집이 S-03 watcher 를 통해 5초 안에 renderer 로 전파 — `emits doc_changed for an external edit within the watcher window` + 9단계 tracer 의 마지막 단계에서 `doc_changed` (`source: "sync"`) 이벤트와 갱신된 rev/sha 를 확인한다.

아직 미증명인 S-08.5 acceptance (사람 손 GUI 검증 전용):

- §13.7.3 9단계 흐름이 실제 Tauri 창과 키보드/마우스 입력으로 1회 통과되는지. 데이터 경로는 위 tracer 테스트로 고정되어 있으므로, 사람 검증은 렌더러 단축키(`Cmd/Ctrl+Enter` 승인)와 슬래시 메뉴 UX 확인에 집중하면 된다.
