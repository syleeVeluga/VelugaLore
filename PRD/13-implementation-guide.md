...
- 현재 `@weki/agent-server` 경로는 Patch/ReadOnlyAnswer 계약과 approval 흐름을 검증하기 위한 로컬 deterministic 구현이며, OpenAI/Anthropic/Gemini API key 없이도 desktop shell launch가 가능하다. D13의 v1 provider 결정은 유지한다.
